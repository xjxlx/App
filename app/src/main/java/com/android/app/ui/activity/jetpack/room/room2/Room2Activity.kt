...
class Room2Activity : BaseBindingTitleActivity<ActivityRoom2Binding>() {
  ...
  override fun onClick(v: View?) {
    ...
    when (v?.id) {
      ...
      R.id.btn_table_1_update -> {
        ...
        RoomUtil.getInstance()
            .execute(
                object : RoomExecuteListener<Int> {
      ...
      R.id.btn_table_1_query -> {
        RoomUtil.getInstance()
            .execute(
                object : RoomExecuteListener<RoomTable1> {
      ...
      R.id.btn_table_2_create -> {
        RoomUtil.getInstance()
            .execute(
                object : RoomExecuteListener<Long> {
        ...
        RoomUtil.getInstance()
            .execute(
                object : RoomExecuteListener<Long> {
                  override fun execute(): Long {
                    val migration =
                        object : Migration(VERSION - 1, VERSION) {
                          override fun migrate(database: SupportSQLiteDatabase) {
                            val addColumn =
                                RoomUtil.getInstance()
